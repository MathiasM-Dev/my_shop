...
class CartController extends AbstractController
{
  ...
  public function index(SessionInterface $session, Cart $cart): Response
    ...
    #[Route("/cart/less/{id}/", name: "app_cart_less", methods: ['GET'])]
    // Définit une route pour enlever un produit au panier

    public function removeProductToCart(int $id, SessionInterface $session): Response
    // Méthode pour enlever un produit au panier, prend l'ID du produit et la session en paramètres

    {
        ...
        // Retire 1 unité à la quantité totale de proquit jusqu'à 0 puis retire l'article du panier
        $session->set('cart',$cart);
        // Met à jour le panier dans la session
        return $this->redirectToRoute('app_cart');
        // Redirige vers la page du panier
